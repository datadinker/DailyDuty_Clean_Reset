 ...
 for (const a of ANSWERS) VALID_SET.add(a);
   ...
   function setDifficultyButtons() {
     ...
     function resetSession() {
         dailyMessageEl.classList.remove("win");
   ...
   function normalizeGuess(raw) {
     ...
     function focusGuessInputIfDesktop() {
   ...
   function onVirtualKey(key) {
     ...
     function renderAnswerInfo(answer, header = "") {
       if (!dailyMessageEl) return;

       const info = (window.WORD_INFO && window.WORD_INFO[answer]) || "";
       ...
       dailyMessageEl.textContent = text;
     }
     ...
     function finalizeLoss() {
       ...
       const answer = state.targetWord || "";
         renderAnswerInfo(answer, "MISSION FAILED - Word was:");
   ...
   // click board/keyboard area to keep focus for hardware typing
     boardEl.addEventListener("click", () => focusGuessInputIfDesktop());
     keyboardEl?.addEventListener("click", () => focusGuessInputIfDesktop());
   ...
   state.difficulty = (savedDiff === Difficulty.PLAYER) ? Difficulty.PLAYER : Difficulty.BEGINNER;
     ...
     setStatus(
       ...
       "Official Daily tracks stats. Practice does not."
     );
